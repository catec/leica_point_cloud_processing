...
def create_training_input_fn(features, labels, batch_size, num_epochs=None, shuffle=True):
    ...
    def _input_fn(num_epochs=None, shuffle=True):
        ...
        ds = Dataset.from_tensor_slices(
            (raw_features, raw_targets))  # warning: 2GB limit
...
def train_linear_classification_model(
    ...
    _ = map(os.remove, glob.glob(os.path.join(
        classifier.model_dir, 'events.out.tfevents*')))
...
training_targets, training_examples = parse_labels_and_features(
    mnist_dataframe[:7500])
validation_targets, validation_examples = parse_labels_and_features(
    mnist_dataframe[7500:10000])
...
# Use a neural network instead classifier
...
test_predictions = np.array([item['class_ids'][0]
