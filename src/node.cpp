...
bool gicpCb(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
{
  iterate = true;
...
bool undoCb(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
{
...
bool fodsCb(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
{
  get_fods = true;
...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    if (new_cad_cloud && new_scan_cloud && ros::ok())
    {
      ...
      if (!Utils::isValidCloud(scan_cloud_aligned))
        return 0;
      ...
      while (new_cad_cloud && new_scan_cloud && ros::ok())
      {
        ...
        if (get_fods)
        {
          ...
          if (!boolean_difference.substract_error)
          {
            ...
            num_of_fods =
                fod_detector.clusterIndicesToROSMsg(cluster_indices, scan_cloud_substracted, cluster_msg_array);
